...
생존 분석에서, 우리가 흔히 하는 통계량들의 통계적 성질, 점근적 분포(Asymptotics) 등을 이해하기 위해서는 Counting Process에 대한 기본 개념이 필요합니다.
따라서 이번 포스트에서는 Counting Process에 대해서 정리할 것입니다.

## Counting Process

먼저, 단일 타입의 사건을 고려하자. $N(t)$를 시간 $t$까지 일어난 사건의 횟수라고 가정하자.

이 때, $N(t)$를 counting process라고 합니다.
...
가 성립하면 됩니다. 여기서 $\lambda$를 intensity라 하는데 이것을 나중에 정의할 것입니다.

## Intensity Process

앞서 언급한 intensity process는 짧은 순간에 사건이 일어날 조건부 확률입니다. 즉,

$$
P(d N(t)=1 \mid \text { past })=\lambda(t) d t
$$

으로 나타낼 수 있습니다. 여기서, $dN(t)$가 binary variable임을 고려하여,
...
를 정의하자. 그러면 $E(dM(t) \mid \text{past} ) = 0$이 성립합니다. 나중에 Martingale 정의할 때, 그 정의와도 비교해보자.
...
식 (3)을, observation  = signal (Hazard) + noise(martingale)로 이해할 수 있습니다.

## Counting Process for survival data

uncensored time $T$ with hazard $\alpha(t)$에서,  counting process 는 $N^c(t)=I(T \leq t)$로 정의합니다. 그러면:

$$
P\left(d N^c(t)=1 \mid \text { past }\right)=P(t \leq T<t+d t \mid \text { past })= \begin{cases}\alpha(t) d t & T \geq t \\ 0 & T<t\end{cases}
$$

위에서, Intensity process는, $\lambda^c(t)=\alpha(t) I(T \geq t)$가 됩니다.

이제, 여러 event를 고려해보자.

uncersored & indep 생존시간 $T\_1, T\_2, \ldots, T\_n$ with hazard $\alpha\_i (t)$에 대해, 각각의 counting process $N\_i ^c (t) = I(T\_i \le t)$에 대해(물론, 이러면 intensity process는 $\lambda\_i ^c (t) = \alpha\_i (t) I(T\_i \ge t)$ 로 주어집니다.),

Aggregated process $N^c(t) = \sum\_{i=1}^n N\_i ^c (t)$는 다음과 같은 intensity process를 가진다:
...
이제, 오른쪽 censoring이 있는 데이터를 고려해보자. 각 개체 $i$마다 censored survival time $\tilde{T\_i}$는 
