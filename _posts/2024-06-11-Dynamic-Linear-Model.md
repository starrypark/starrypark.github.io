...
$$
\begin{equation}\begin{aligned}&\left(\mathbf{Y}_t \mid \boldsymbol{\theta}_t\right) \sim \mathrm{N}\left[\mathbf{F}_t^{\prime} \boldsymbol{\theta}_t, \mathbf{V}_t\right]\\&\left(\boldsymbol{\theta}_t \mid \boldsymbol{\theta}_{t-1}\right) \sim \mathrm{N}\left[\mathbf{G}_t \boldsymbol{\theta}_{t-1}, \mathbf{W}_t\right]\end{aligned}\end{equation}
$$

로 나타낼 수 있다.  혹은,

$$
\begin{equation}\begin{aligned}\mathbf{Y}_t=\mathbf{F}_t^{\prime} \boldsymbol{\theta}_t+\boldsymbol{\nu}_t, & \boldsymbol{\nu}_t \sim \mathrm{N}\left[\mathbf{0}, \mathbf{V}_t\right], \\\boldsymbol{\theta}_t=\mathbf{G}_t \boldsymbol{\theta}_{t-1}+\boldsymbol{\omega}_t, & \boldsymbol{\omega}_t \sim \mathrm{N}\left[\mathbf{0}, \mathbf{W}_t\right]\end{aligned}\end{equation}
$$
...
$$
\begin{equation}\begin{array}{lcl}\text { Observation equation: } & Y_t=\mathbf{F}_t^{\prime} \boldsymbol{\theta}_t+\nu_t, & \nu_t \sim \mathrm{N}\left[0, V_t\right], \\\text { System equation: } & \boldsymbol{\theta}_t=\mathbf{G}_t \boldsymbol{\theta}_{t-1}+\boldsymbol{\omega}_t, & \boldsymbol{\omega}_t \sim \mathrm{N}\left[\mathbf{0}, \mathbf{W}_t\right], \\\text { Initial information: } & \left(\boldsymbol{\theta}_0 \mid D_0\right) \sim \mathrm{N}\left[\mathbf{m}_0, \mathbf{C}_0\right], &\end{array}\end{equation}
$$
...
1. Posterior at $t-1$
    ...
    $$
    \begin{equation}\left(\boldsymbol{\theta}_{t-1} \mid D_{t-1}\right) \sim \mathrm{N}\left[\mathbf{m}_{t-1}, \mathbf{C}_{t-1}\right]\end{equation}
    $$
    
2. Prior at $t$
    
    $$
    \left(\boldsymbol{\theta}_t \mid D_{t-1}\right) \sim \mathrm{N}\left[\mathbf{a}_t, \mathbf{R}_t\right]
    $$
...
4. Posterior at $t$
    ...
    $$
    \left(\boldsymbol{\theta}_{t} \mid D_{t}\right) \sim \mathrm{N}\left[\mathbf{m}_{t}, \mathbf{C}_{t}\right]
    $$
...
$$
\begin{equation}f_t(k)=\mathrm{E}\left[\mu_{t+k} \mid D_t\right]=\mathrm{E}\left[\mathbf{F}_{t+k}^{\prime} \boldsymbol{\theta}_{t+k} \mid D_t\right]\end{equation}
$$

여기서 $\mu_v=\mathbf{F}_v^{\top} \boldsymbol{\theta}_v$는 mean response function이라고 부른다.

### Theorem 4.2

$0 \le j <k$인 모든 $j$에 대해, 각 time point $t$에서 다음이 성립한다.

- (a) State distribution : $\left(\boldsymbol{\theta}_{t+k} \mid D_t\right) \sim \mathrm{N}\left[\mathbf{a}_t(k), \mathbf{R}_t(k)\right]$
- (b) Forecast distribution :  $\left(Y_{t+k} \mid D_t\right) \sim \mathrm{N}\left[f_t(k), Q_t(k)\right]$
- (c) State Covariance : $\mathrm{C}\left[\boldsymbol{\theta}_{t+k}, \boldsymbol{\theta}_{t+j} \mid D_t\right]=\mathbf{C}_t(k, j)$
- (d) Obsn. Covariance : $\mathrm{C}\left[Y_{t+k}, Y_{t+j} \mid D_t\right]=\mathbf{F}_{t+k}^{\prime} \mathbf{C}_t(k, j) \mathbf{F}_{t+j}$
- (e) Other Covariance : $\begin{aligned}& \mathrm{C}\left[\boldsymbol{\theta}_{t+k}, Y_{t+j} \mid D_t\right]=\mathbf{C}_t(k, j) \mathbf{F}_{t+j} \\& \mathrm{C}\left[Y_{t+k}, \boldsymbol{\theta}_{t+j} \mid D_t\right]=\mathbf{F}_{t+k}^{\prime} \mathbf{C}_t(k, j)\end{aligned}$
...
- Proof of Theorem 4.2
    ...
    여기서 $\boldsymbol{\theta}_t=\mathbf{G}_t \boldsymbol{\theta}_{t-1}+\boldsymbol{\omega}_t$를 반복적으로 적용하여 계산할 시,
    
    $$
    \boldsymbol{\theta}_{t+k}=\mathbf{H}_{t+k}(k) \boldsymbol{\theta}_t+\sum_{r=1}^k \mathbf{H}_{t+k}(k-r) \boldsymbol{\omega}_{t+r}
    $$
    
    를 얻을 수 있다. 이 식을 통해, 
    
    $$
    \left(\boldsymbol{\theta}_{t+k} \mid D_t\right) \sim \mathrm{N}\left[\mathbf{a}_t(k), \mathbf{R}_t(k)\right]
    $$
...
- Proof of Theorem 4.4 (미완)
    ...
    $$
    \left(\boldsymbol{\theta}_{t-k+1} \mid D_t\right) \sim \mathrm{N}\left[\mathbf{a}_t(-k+1), \mathbf{R}_t(-k+1)\right]
    $$
    ...
    $$
    p\left(\boldsymbol{\theta}_{t-k} \mid \boldsymbol{\theta}_{t-k+1}, D_t\right)=\frac{p\left(\boldsymbol{\theta}_{t-k} \mid \boldsymbol{\theta}_{t-k+1}, D_{t-k}\right) p\left(\mathbf{Y} \mid \boldsymbol{\theta}_{t-k}, \boldsymbol{\theta}_{t-k+1}, D_{t-k}\right)}{p\left(\mathbf{Y} \mid \boldsymbol{\theta}_{t-k+1}, D_{t-k}\right)}
    $$
    
    다음과 같은 식을 얻을 수 있다. 여기서 $\mathbf{Y}=\left\{Y_{t-k+1}, \ldots, Y_t\right\}$이다. 여기서 $\boldsymbol{\theta}_{t-k+1}$가 주어져 있는 경우, $\boldsymbol{Y} \perp \boldsymbol{\theta}_{t-k}$의 조건부 독립을 이용하여, 뒤의 항은 cancel-out이 가능하고,
    ...
    $$
    \begin{equation}\left(\boldsymbol{\theta}_{t-k+1} \mid \boldsymbol{\theta}_{t-k}, D_{t-k}\right) \sim \mathrm{N}\left[\mathbf{G}_{t-k+1} \boldsymbol{\theta}_{t-k}, \mathbf{W}_{t-k+1}\right]\end{equation}
    $$
    
    이고,
    
    $$
    \begin{equation}\left(\boldsymbol{\theta}_{t-k} \mid D_{t-k}\right) \sim \mathrm{N}\left[\mathbf{m}_{t-k}, \mathbf{C}_{t-k}\right]\end{equation}
    $$
    
    이다. 여기서, Joint distribution을 정의하고 그것의 conditional mean과 variance를 구하는 방법으로, 식 (7)에서,
    
    $$
    \begin{equation}\left(\boldsymbol{\theta}_{t-k} \mid \boldsymbol{\theta}_{t-k+1}, D_{t-k}\right) \sim \mathrm{N}\left[\mathbf{h}_t(k), \mathbf{H}_t(k)\right]\end{equation}
    $$
