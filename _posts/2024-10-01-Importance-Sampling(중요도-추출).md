...
이 포스팅은 중요도 추출에 대한 내용입니다. 베이지안 샘플링이나, 딥러닝 일부 알고리즘에서도 정말 중요하게 쓰이는 알고리즘이니 꼭 알아두자.
...
- Importance Sampling은 $\mathbb{E}[f]$를 $p(x)$를 잘 모르는 상태에서 계산하기 위해 사용합니다.
- 대신, 표본 $\left\\{x\_n\right\\}$은 더 간단한 분포 $q(x)$에서 추출합니다. 이 때 $q(x)>0$ if $p(x)>0$이고, corresponding term이 $\frac{p\left(x\_n\right)}{q\left(x\_n\right)}$의 가중치를 가지며 importance weight라고 합니다.
...
1. $\theta\_1 , \cdots,\theta\_m \sim q(\theta)$를 추출합니다.
2. 가중치 $w\_i = \frac{p(\theta\_i)}{q(\theta\_i)}$를 추출합니다.
3. $\mathbb{E}[f]$를 다음 두 값중 하나로 추정합니다.